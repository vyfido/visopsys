...
int kernelDescriptorSet(volatile kernelSelector selector, 
			volatile void *base, unsigned int size, 
			int present, int privilegeLevel, int system, 
			int type, int granularity, int bitSize)
{
  // This function is mostly internal, and is used to change a descriptor
  ...
  globalDescriptorTable[entryNumber].baseAddress2 = (unsigned char)
    (((unsigned int) base & 0x0000FF00) >> 8);
  globalDescriptorTable[entryNumber].baseAddress3 = (unsigned char)
    (((unsigned int) base & 0x00FF0000) >> 16);
  ...
  globalDescriptorTable[entryNumber].baseAddress4 = (unsigned char)
    (((unsigned int) base & 0xFF000000) >> 24);
...
int kernelDescriptorChangePrivilege(kernelSelector selector, int newPrivilege)
{
  // This function will change the privilege level of a basic code, data,
  // or stack descriptor in the requested table.  Returns 0 on success, 
  // negative otherwise
  ...
  // Make sure the new privilege we're setting is legal
  if ((newPrivilege != PRIVILEGE_USER) && 
      (newPrivilege != PRIVILEGE_SUPERVISOR))
    return (status = ERR_INVALID);

  // Set the new privilege level
  globalDescriptorTable[entryNumber].attributes1 &= 0x9F;
  globalDescriptorTable[entryNumber].attributes1 |= (newPrivilege << 5);
...
void *kernelDescriptorGetIDTInterruptVector(int number)
{
  // This function can be used to get the current interrupt vector contained
  // in the requested interrupt gate.  Note that this function does NOT
  // check to make sure that the target descriptor actually contains a
  // valid interrupt gate.  Returns NULL on error.

  unsigned int temp = 0;
  void *interruptVector = NULL;
  ...
  // Now we can begin constructing the address from the requested
  // descriptor.

  temp = (interruptDescriptorTable[number].baseAddress4 << 24);
  temp += (interruptDescriptorTable[number].attributes2 << 16);
  temp += (interruptDescriptorTable[number].segSizeByte2 << 8);
  temp += interruptDescriptorTable[number].segSizeByte1;

  return (interruptVector = (void *) temp);
}
