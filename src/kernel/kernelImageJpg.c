...
static int readBlock(unsigned char *dataPointer, unsigned *bitPosition,
		     jpgHuffTable *dcTable, jpgHuffTable *acTable,
		     short *dcValue, short *coeff)
{
  ...
  if (status < 0)
    {
      kernelDebugError("Can't decode DC category for bits %04x offset %u:%d",
		       readBits(dataPointer, bitPosition, 16, 0),
		     ...
		     short *crCoeff, int xCoord, int yCoord, pixel *pixels)
{
  ...
  for (count1 = 0; count1 < mcuHeight; count1 ++)
      ...
      for (count2 = 0; count2 < mcuWidth; count2 ++)
	{
	  ...
	  green =
	    rgbG(yCoeff[coeffIndex], cbCoeff[coeffIndex], crCoeff[coeffIndex]);
	  green = min(green, 255);
	  green = max(green, 0);
	  ...
	  for (count = 0; count < Y_BLOCKSPERMCU; count ++)
	      ...
	      if (status < 0)
		{
	      ...
	      deQuantBlock((yCoeff + (count * 64)),
			   &jpg->quantTable[jpg->frameHeader->comp[0]
					    .quantTable]);
	  ...
	  for (count = 0; count < CB_BLOCKSPERMCU; count ++)
	      ...
	      if (status < 0)
		{
	      ...
	      deQuantBlock((cbCoeff + (count * 64)),
			   &jpg->quantTable[jpg->frameHeader->comp[1]
					    .quantTable]);
	  ...
	  for (count = 0; count < CR_BLOCKSPERMCU; count ++)
	      ...
	      if (status < 0)
		{
	      ...
	      deQuantBlock((crCoeff + (count * 64)),
			   &jpg->quantTable[jpg->frameHeader->comp[2]
					    .quantTable]);
      ...
      if ((imageFileData[count1] == 0xFF) && (count1 < (dataLength - 1)))
	{
	  ...
	  switch (imageFileData[count1])
	      ...
	      while (jpg->numQuantTables < JPG_QUANT_TABLES)
		{
		  ...
		  // header
		  status =
		    genQuantTable((imageFileData[count2] >> 4),
