  ...
  while (kernelWindowEventStreamRead(&mouseEvents, &event) > 0)
    {
      ...
      // Shortcut: If we were dragging a component
...
static int spawnWindowManagerThread(void)
{
...
{
  // Get the list of programs to run automatically and run them.
  ...
  // Get the list of programs we're supposed to run
  if (!kernelVariableListGet(settings, "programs", programList, 128))
    {
      ...
      while(1)
	{
	  ...
	  count = 0;
	  while ((programName[count] != '\0') && (programName[count] != ','))
	    count++;

	  // Get the rest of the recognized properties for this icon.
	  strcpy(propertyName, "program.");
	  strncat(propertyName, programName, count);
	  kernelVariableListGet(settings, propertyName, command, 128);

	  // Try to run the program
	  kernelLoaderLoadAndExec(command,
				  kernelUserGetPrivilege(loggedInUser),
				  0, NULL, 0);

	  // Move to the next program name, if applicable
...
void kernelWindowProcessEvent(windowEvent *event)
{
  ...
  if (event->type & EVENT_MASK_MOUSE)
      ...
      if (draggingComponent && (event->type == EVENT_MOUSE_DRAG))
	{
	  ...
	  // Put this mouse event into the component's windowEventStream
	  kernelWindowEventStreamWrite(&(draggingComponent->events),
				       event);
	  // We can stop here	
	  return;
	}
  ...
  if (dialog)
    {
      ...
      label = kernelWindowNewTextLabel(dialog, labelText, &params);
						...
						componentParameters *params)
{
  ...
  if (((kernelWindow *) parent)->type == windowType)
	...
	status = ((kernelWindowContainer *) tmpWindow->mainContainer->data)
      ...
      component->container = (void *) tmpWindow->mainContainer;
    }
	...
	->containerAdd(tmpComponent, component, params);
      component->container = (void *) parent;
    }
  else
    {
