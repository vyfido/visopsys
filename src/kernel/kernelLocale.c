  ...
  if (!strcmp(locale, ""))
    {
      ...
      // variables.
      status = kernelEnvironmentGet(name, buffer, 16);
      if (status >= 0)
	strncpy(category, buffer, 15);
      else
...
static int loadMessageFile(void)
{
  ...
  sprintf(path, "%s/%s/LC_MESSAGES/kernel.mo", GETTEXT_LOCALEDIR_PREFIX,
	  _lc_messages);
  ...
  strncpy(msgFile->path, path, MAX_PATH_NAME_LENGTH);

  status =
    kernelFileRead(&fileStructure, 0, fileStructure.blocks, msgFile->buffer);
  if (status < 0)
    goto out;
...
char *kernelGetText(const char *msgid)
{
  ...
  for (count = 0; count < msgFile->header->numStrings; count ++)
    if (!strcmp((msgFile->buffer + msgFile->origTable[count].offset), msgid))
      {
