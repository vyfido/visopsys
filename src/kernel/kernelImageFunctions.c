...
int kernelImageLoadBmp(const char *filename, image *loadImage)
{
  ...
  if (strncmp(imageFileData, "BM", 2))
    {
      ...
      kernelMemoryReleaseBlock(imageFileData);
  ...
  imageData = kernelMemoryRequestBlock(loadImage->dataLength, 0, "image data");
  ...
  if (header->bitsPerPixel == BMP_BPP_24BIT)
	  ...
	  for (count = (height - 1); count >= 0; count --)
		   ...
		   pixelRowCounter++)
		{
		  ...
		  if (colorIndex >= colors)
		    {
		      // Error.  Release the file data memory
		      kernelMemoryReleaseBlock(imageFileData);
		      return (status = ERR_INVALID);
		    }
      ...
      else
	{
	  ...
	  kernelMemoryReleaseBlock(imageFileData);
	  kernelMemoryReleaseBlock(imageData);
	  return (status = ERR_INVALID);
	}
    }
  else
...
int kernelImageSaveBmp(const char *filename, image *saveImage)
{
  ...
  fileData = kernelMemoryRequestBlock(header.size, 0, "temp file data");
